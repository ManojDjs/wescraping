...
url = 'https://jobs.perkinelmer.com/search-jobs'
urlsarray=[]
driver.get(url)
for i in range(1,8):
    time.sleep(15)
    ...
    for l in link:
        ...
        location=l.find_element_by_xpath('span').text
        print(title)
        print(joburl)
        print(location)
        d={}
        ...
        d['location']=location
        d['url']=joburl
        d['company']="PerkinElmer"
        urlsarray.append(d)
        print(urlsarray)
    driver.find_element_by_xpath('//section[@class="pagination"]/div[@class="pagination-buttons"]/div/a[@class="next"]').click()
    print(urlsarray)
    print(len(urlsarray))
